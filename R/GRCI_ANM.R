GRCI_ANM <- function(X,Y,C,Xp,outL=NULL){
  require(RANN)
  require(gtools)
  ...
  if (is.null(outL)){
    ...
    outL = DirectHNM_fast_Y_ANM(X,Y,C,Xp,G)
...
DirectHNM_fast_Y_ANM <- function(X,Y,C,Xp,G){
  X = as.matrix(X)
  # E = X
  ...
  repeat{ 
    s_out = FindSink_ANM(X,U,S,C,Xp,G,update)
    sink = s_out$sink
    S = s_out$S
    ...
    if (sum(G[sink,])){
      ...
      X[,sink] = nb_Pearson_ANM(X[,G[sink,],drop=FALSE],X[,sink],C,Xp)
    }
...
FindSink_ANM <- function(X,U,S,C,Xp,G,update,w=NULL){
  r = length(update)
  ...
  for (i in seq_len(r)){
    ...
    S[update[i]] = CompareG_ANM(X,C,Xp,update[i],V,w) # baseline
...
CompareG_ANM <- function(X,C,Xp,i,js,w=NULL){
  ...
  rXY = nb_Pearson_ANM(X[,js,drop=FALSE],X[,i],C,Xp)
...
nb_Pearson_ANM <- function(X,Y,C,Xp){
