...
Gtr = matrix(0,6,7)
Gtr[1,4]=1; Gtr[2,3]=1; Gtr[3,4]=1;
Gtr[4,5]=1; Gtr[1,6]=1; Gtr[,7]=1
n = nrow(dataf1)
p = ncol(dataf1)
...
for (i in 1:50){
  ...
  shapsT = RCI_NB_G(cbind(dataf1,Y),ncol(dataf1)+1,C/length(C),as.matrix(Xp),Gtr=Gtr)$shaps
  
  ib = sample(1:n,n,replace=TRUE)
  dataft = cbind(dataf1[ib,],Y[ib])
  Ct = C[ib]; Xpt = Xp[ib,]
  ...
  ptm <- proc.time()
  out = ICA_predict_RMSE(datan,dataft[,p+1])
  ...
  save(file="Results_Laughney.RData", RCINB_res, RCI_res, GRCI_ANM_res,
       ICA_res, GRCI_HNM_res)
}
